...
export default function ArticlesPage() {
  ...
  async function handleBatchAnalyzeSelected() {
    ...
    await mutations.batchAnalyzeArticles.mutateAsync(selectedIds);
    setSelectedIds([]);
  }

  async function handleBatchAnalyzeByQuery() {
    await mutations.batchAnalyzeArticlesByQuery.mutateAsync({
      sourceId: sourceId || undefined,
      q: deferredQuery.trim() || undefined,
      dateFrom: dateFrom || undefined,
      dateTo: dateTo || undefined,
  ...
  return (
    <PageFrame
      title="文章浏览"
      subtitle="专注文章摘要、文章标签与收藏状态。这里的标签筛选只作用于文章标签，不混入公众号来源标签。"
      actions={
        <>
          ...
          <ActionButton
            ...
            disabled={!selectedIds.length || mutations.batchAnalyzeArticles.isPending}
          >
            <Sparkles size={14} className="mr-2" />
            总结已选文章
          </ActionButton>
    ...
    >
      <section className="rounded-[30px] border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.06),rgba(255,255,255,0.03))] p-5 shadow-glow">
        <div className="grid gap-5 xl:grid-cols-[minmax(0,1.45fr)_minmax(360px,0.95fr)]">
          <div className="rounded-[26px] border border-white/10 bg-black/20 p-5">
            ...
            <div className="mt-4 flex flex-col gap-3 lg:flex-row">
              <Input
                ...
                className="h-12 flex-1 rounded-[22px] bg-white/6"
              />
              <ActionButton variant="ghost" onClick={() => articles.refetch()} className="h-12 px-5">
                搜索
              </ActionButton>
            </div>
          ...
          <div className="rounded-[26px] border border-white/10 bg-black/20 p-5">
            <div className="flex items-center justify-between gap-3">
              ...
              <label className="inline-flex items-center gap-3 rounded-full border border-white/10 bg-white/5 px-4 py-2 text-sm text-white/75">
                <input
                  type="checkbox"
            ...
            <div className="mt-5 rounded-[20px] border border-dashed border-white/10 bg-white/4 px-4 py-3 text-sm text-white/52">
              当前条件下共 {total} 篇文章，可直接批量总结未总结文章，也可以先选择部分文章再执行。
            </div>
          </div>
        </div>
        ...
        <div className="mt-5 rounded-[26px] border border-white/10 bg-black/25 p-5">
          <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm leading-6 text-white/52">
                参考 flomo 的标签工作流：先输入，再从候选中选择。这里展示的是文章标签体系和已使用过的文章标签，不包含来源标签。
              </p>
            </div>
            {selectedTags.length ? (
              <ActionButton
          ...
          <div className="relative mt-5">
            <div className="rounded-[22px] border border-white/12 bg-white/6 px-4 py-3">
              <input
                value={tagQuery}
            ...
            {tagPickerOpen ? (
              <div className="absolute left-0 right-0 z-20 mt-2 rounded-[22px] border border-white/12 bg-[#0d1524]/95 p-3 shadow-2xl backdrop-blur">
      ...
      <section className="mt-8">
        <SectionTitle title="文章列表" subtitle="卡片里只展示文章标签；来源标签保留在来源管理和文章详情的来源信息中。" />
        {articles.isLoading ? (
          ...
          <>
            ...
            <div className="grid gap-4 xl:grid-cols-2">
              {currentItems.map((article) => (
                <div
                  key={article.id}
                  className="rounded-[28px] border border-white/10 bg-[linear-gradient(180deg,rgba(12,18,32,0.92),rgba(10,14,24,0.78))] p-5 shadow-[0_20px_60px_rgba(0,0,0,0.22)]"
                >
                  ...
                  {article.summary ? (
                    <div className="mt-4 rounded-[20px] border border-white/8 bg-white/4 px-4 py-3">
                      <p className="whitespace-pre-wrap break-words text-sm leading-7 text-white/68">{article.summary}</p>
                    </div>
                  ) : null}
                  <div className="mt-4">
            ...
            <div className="mt-6 flex flex-wrap items-center justify-between gap-3">
              ...
              <p className="text-sm text-white/60">当前条件下共 {total} 篇文章</p>
              <ActionButton
                variant="ghost"

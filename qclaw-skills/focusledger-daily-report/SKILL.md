...
description: Generate a FocusLedger WeChat daily report for a specific date by calling the local FocusLedger API. Use when the user asks for a 公众号日报、某天的日报、今天/昨天/前天的日报, or asks QClaw to summarize FocusLedger content for a date. Do not use for syncing sources, browsing article lists, or source management.
---
...
```powershell
python E:\QClaw\resources\openclaw\config\skills\focusledger-daily-report\scripts\fetch_daily_report.py --date "2026-03-20" --style brief
```

With source group:

```powershell
python E:\QClaw\resources\openclaw\config\skills\focusledger-daily-report\scripts\fetch_daily_report.py --date "昨天" --group "投研/AI" --style brief
```
